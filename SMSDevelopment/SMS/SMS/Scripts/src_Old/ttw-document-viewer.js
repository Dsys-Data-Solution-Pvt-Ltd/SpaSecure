(function ($) {
    ...
    var DocumentViewer = function ($anchor, userOptions) {
        var $wrapper,
            ...
            imageExtensions = ['png', 'jpg', 'jpeg', 'gif', 'tiff', 'tif', 'bmp'],
        ...
        function load(filename, userOptions, fullFilePath) {
            debugger;
            var loadOptions;
            ...
            if (currentType == "conversion") {
                $wrapper.removeClass('pdf code txt video audio image').addClass("pdf");
            }
            else {
            ...
            switch (currentType) {
                ...
                case 'conversion':
                    debugger;
                    var pdf = new ConversionLoader(filename, loadOptions, fullFilePath);
                    break;
                default:
            ...
            if (currentType === 'pdf' || currentType === "conversion") {
        ...
        var TextLoader = function (filename, options) {
            /*if (window.location.toString().toLowerCase().indexOf("adminpages") > -1) {
            var url = "../../Handler/getContents.ashx";
            }
            else {
            var url = "../Handler/getContents.ashx";
            }*/
            ...
            debugger;
            $.ajax({
                url: url, //options.path + 'libs/getContents.php',
                type: 'POST',
                data: { file: filename },
        ...
        var ConversionLoader = function (filename, options, fullFilePath) {
            /*if (window.location.toString().toLowerCase().indexOf("adminpages") > -1) {
            var url = "../../Handler/getContents.ashx";
            }
            else {
            ...
            $.ajax({
                ...
                success: function (response) {
                    debugger;
                    fullFilePath = fullFilePath.replace("." + options.extension, "_new.pdf");
                    options.type = "pdf";
                    load(fullFilePath, options, fullFilePath)

                    /* var pdf = new PDFLoader(fullFilePath, options);
                    initScrollbar("pdf");
                    triggerLoadedEvent(options.loadEventId);*/
                },
                error: function (e) {
